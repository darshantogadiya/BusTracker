...
export default class ntoawithPatel extends React.Component {
    constructor() {
        super();
        this.state = {
    ...
    componentDidMount() {
        ...
        setInterval(() => {
            ...
            if (new Date().getHours() >= 2 && new Date().getMinutes() >= 0) {
                this.setState({
                    ahmedabad: false, nadiad : true, kheda: false, delayedTime: 5
                })
            }
            else if (new Date().getHours() >= 2 && new Date().getMinutes() >= 20) {
                this.setState({
                    ahmedabad: false, nadiad : false, kheda: true, delayedTime: 10
                })
            }
            else if (new Date().getHours() >= 2 && new Date().getMinutes() >= 40) {
                this.setState({
                    ahmedabad: true, nadiad : false, kheda: false, delayedTime: 0
                })
            }
            else {
                ...
                <View style={{
                    ...
                    <View style={{ alignItems: "center", marginHorizontal: 15, elevation: 5, marginTop: 20, backgroundColor: "#F3EFEF", padding: 20, borderRadius: 10 }}>
                        ...
                        <View style={{ flexDirection: "row", width: "100%" }}>
                            <View style={{ width: "32%", alignItems: "center" }}>
                                ...
                                <Text style={{ color: "green", fontWeight: "bold", fontSize: 10 }}>02:00 PM</Text>
                                <Text style={{ color: "green", fontWeight: "bold", fontSize: 10 }}>Nadiad</Text>
                            </View>
                            <View style={{ width: "2%" }}></View>
                            <View style={{ width: "32%", alignItems: "center" }}>
                                <Text style={{ marginLeft: 5, fontWeight: "bold" }}>|</Text>
                                <Text style={{ color: "grey", fontWeight: "bold", fontSize: 10 }}>02:20 PM</Text>
                                <Text style={{ color: "grey", fontWeight: "bold", fontSize: 10 }}>Kheda</Text>
                            </View>
                            <View style={{ width: "2%" }}></View>
                            <View style={{ width: "32%", alignItems: "center" }}>
                                <Text style={{ marginLeft: 5, fontWeight: "bold" }}>|</Text>
                                <Text style={{ color: "red", fontWeight: "bold", fontSize: 10 }}>02:40 PM</Text>
                    ...
                    {this.state.nadiad &&
                        ...
                        <MapView
                            provider={PROVIDER_GOOGLE}
                            style={{ flex: 1, marginHorizontal: 15, elevation: 5, marginTop: 20, backgroundColor: "#F3EFEF", borderRadius: 20, marginBottom: 20 }}
                            region={this.state.initialRegionKheda}
                            zoomEnabled={true}
                            pitchEnabled={true}
                            showsCompass={true}
                            showsBuildings={true}
                            ...
                            <Marker
                                coordinate={{
                                    latitude: this.state.initialRegionKheda.latitude,
                                    longitude: this.state.initialRegionKheda.longitude,
                                }}>
                            </Marker>
